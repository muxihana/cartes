...
> **MCP fork 開發中：** `feat/mcp-server` 分支新增一個本機 STDIO MCP Server，讓 AI 以玩家身分加入牌桌。發牌與洗牌由伺服器掌控，AI 只會收到自己的手牌與規則允許公開的莊家明牌；安裝方式、工具與安全界線請看 [`docs/MCP.md`](docs/MCP.md)。原本的單檔網頁仍維持不變。
