...
這是一張完全在瀏覽器裡運作的單檔牌桌：有完整的 21 點與十點半規則、純 CSS 牌面、分模式戰績、角色台詞、求救與桌邊聊天。它能讀取酒館 PNG／JSON 角色卡，也接受 Markdown、純文字或直接手填。對話最多保留最近 60 句，凌晨四點才換日。

## 開玩三步

1. 打開牌桌網頁，按右上角「設定」。
2. 填入 OpenAI 相容 API 的端點、金鑰與模型名；不確定模型名稱時可按「撈清單」。
...
也可以使用 OpenRouter：把它提供的 OpenAI 相容端點、金鑰與模型名填進同樣三格即可。請只使用你信任的 API 供應商，也不要把金鑰貼到公開貼文、issue、截圖或提交紀錄裡。
...
- 資料存在目前瀏覽器；換瀏覽器、換裝置、使用無痕模式或清除網站資料後，角色記憶不會跟著搬家。
